...
Route::get('/contratos-estado/{dep}/{prov?}/{dist?}', function ($dep, $prov = null, $dist = null) {
    $nombreDep = ucwords(str_replace('-', ' ', $dep));
    $nombreProv = $prov ? ucwords(str_replace('-', ' ', $prov)) : null;
    $nombreDist = $dist ? ucwords(str_replace('-', ' ', $dist)) : null;

    return view('buscador-publico-regional', [
        'seoDep'       => $dep,
        'seoProv'      => $prov,
        'seoDist'      => $dist,
        'nombreDep'    => $nombreDep,
        ...
        'departamentos' => departamentosSidebar(),
    ]);
})->name('contratos.estado');

Route::get('/buscador-publico/{dep}/{prov?}/{dist?}', function ($dep, $prov = null, $dist = null) {
    $nombreDep = ucwords(str_replace('-', ' ', $dep));
    $nombreProv = $prov ? ucwords(str_replace('-', ' ', $prov)) : null;
    $nombreDist = $dist ? ucwords(str_replace('-', ' ', $dist)) : null;
...
})->name('buscador.departamental');

Route::get('/buscador-contratos-mayores', function () {
    return view('buscador-mayores');
})->name('buscador.mayores');

Route::get('/buscador-contratos-mayores/{entidad}', function ($entidadSlug) {
    // Buscar entidad desde DB por nombre normalizado
    $entidad = Cache::remember("entidad_mayor_slug:{$entidadSlug}", 86400, function () use ($entidadSlug) {
        $todas = \App\Models\EntidadMayor::orderBy('nombre')->get(['nombre', 'ruc']);
        foreach ($todas as $e) {
            ...
            $slug = trim($slug, '-');
            if ($slug === $entidadSlug) {
                return ['nombre' => $e->nombre, 'ruc' => $e->ruc, 'slug' => $slug];
            }
        }
        return null;
    });
    ...
    return view('buscador-mayores-regional', [
        'entidadNombre' => $entidad['nombre'],
        'entidadRuc'    => $entidad['ruc'],
        'entidadSlug'   => $entidad['slug'],
        'entidades'     => entidadesMayoresSidebar(),
    ]);
    ...
    $xml = Cache::remember('sitemap_xml', 3600, function () {
        ...
        try {
            ...
            foreach ($departamentos['data'] ?? [] as $depto) {
                ...
                if ($slug) {
                    $urls[] = [
                        'loc' => $baseUrl . '/contratos-estado/' . $slug,
                        'lastmod' => $now,
                        'changefreq' => 'daily',
                        'priority' => '0.9',
                    ];
            ...
            // Contratos Mayores por entidad (desde DB)
            try {
                $entidades = \App\Models\EntidadMayor::orderBy('nombre')->get(['nombre']);
                foreach ($entidades as $e) {
                    ...
                    if ($slug) {
                        $urls[] = [
                            'loc' => $baseUrl . '/buscador-contratos-mayores/' . $slug,
                            'lastmod' => $now,
                            'changefreq' => 'daily',
                            'priority' => '0.9',
                        ];
                ...
                \Illuminate\Support\Facades\Log::warning('Sitemap: Error cargando entidades', ['error' => $e->getMessage()]);
            }
