    ...
    <div class="bg-white rounded-3xl shadow-soft p-4 sm:p-8 border border-neutral-100">
        ...
        <div class="space-y-3">
            ...
            <div wire:key="adjud-toggle-{{ $alertaAdjudicaciones ? '1' : '0' }}"
                 ...
                 class="bg-amber-50/60 border border-amber-100 rounded-2xl px-4 py-3">
                ...
                @if($alertaAdjudicaciones)
                <div class="mt-3 pt-3 border-t border-amber-100">
                    <label class="block text-xs font-medium text-neutral-600 mb-1.5">Monto mínimo de alerta (S/)</label>
                    <div class="flex items-center gap-2">
                        <input type="number" min="1" step="1000" wire:model.live.debounce.800ms="alertaAdjudicacionesUmbral"
                               class="w-full sm:w-64 px-4 py-2.5 rounded-xl text-sm bg-white border border-amber-200 focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <span class="text-[11px] text-neutral-400">Solo se te alertará de procesos con monto igual o mayor.</span>
                    </div>
                </div>
                @endif
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-soft p-4 sm:p-8 border border-neutral-100">
        ...
        @elseif(!$showWhatsAppModal)
            <div class="text-center py-8">
                ...
                <p class="text-xs text-neutral-500 mb-4">Configura tu numero para recibir alertas automaticas.</p>

                @php $waBusinessPhone = config('services.whatsapp.business_phone', ''); @endphp
                @if($waBusinessPhone)
                <div class="bg-amber-50 rounded-2xl p-4 text-left max-w-sm mx-auto">
                    <p class="text-xs text-amber-800 font-semibold mb-2">⚠️ Importante: ventana de 24 horas</p>
                    <p class="text-xs text-amber-700 mb-3">WhatsApp solo permite recibir notificaciones si has enviado al menos 1 mensaje al bot en las ultimas 24 horas.</p>
                    <a href="https://wa.me/{{ $waBusinessPhone }}?text=Hola%20Vigilante%20SEACE"
                       target="_blank"
                       class="inline-flex items-center gap-2 px-4 py-2 bg-green-500 text-white text-xs font-semibold rounded-full hover:bg-green-600 transition-colors">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/></svg>
                        Enviar mensaje al Bot
                    </a>
                    <p class="text-[10px] text-amber-600 mt-2">+{{ $waBusinessPhone }} · Se abre en WhatsApp</p>
                </div>
                @endif
            </div>
        @endif
